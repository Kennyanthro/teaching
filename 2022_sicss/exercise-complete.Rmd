...
*Cars* is a film about, you guessed it, cars [@Wallis-Wallis_2015]. 
...
There are `r num_rows` rows in the cars dataset.
...
The average speed is `r average_speed` mph.

<!-- (task 7) Plot speed against distance (extension: add a regression line) -->
<!-- (task 8) Add caption -->
<!-- (task 9) Hide any messages (hint: it's code chunk option) -->

```{r cars-plot, fig.cap = "Graph showing speed against stopping distance", message = FALSE}
...
We used the `cars` dataset, as reported in Section \@ref(results), and found something groundbreaking. As seen in Figure \@ref(fig:cars-plot) and in Table \@ref(tab:cars-table), as speed increases, stopping distance increases.
